...
case "lastTime":return false;case "tmpSpeed":return false;case "tmpCoord":return false;case "position":return false;case "coord":return false;default:return true}};$scope.startNewGame=function(){if($rootScope.login){var newGameURL="/startnewgame/"+$rootScope.playername+"/"+$rootScope.playerlife+"/"+$rootScope.playermoney+"/"+$rootScope.playeremail+"/"+$rootScope.playingfieldsize+"/"+$rootScope.playingfieldsize;$scope.message=newGameURL;var response=$http.post(newGameURL,$scope.gameContext);response.success(function(dataFromServer,
status,headers,config){$scope.gameContext=dataFromServer;alert($rootScope.playername+": Click OK to start the game");$scope.intervalFunction()});response.error(function(data,status,headers,config){alert("Can't create new game. Sorry !")})}else{alert("You need to Login");$location.path("/")}};$scope.intervalFunction=function(){updateTimer=$timeout(function(){updateFunction("/update");$scope.intervalFunction()},updateTime)};$scope.startNewGame();$scope.$on("$destroy",function(){$timeout.cancel(updateTimer)})});
towerdefenceApp.controller("LoginCtrl",function($rootScope,$scope,$http,$routeParams,$location){function checkLogin(){$rootScope.login=true}function copyLogginDataToGlobal(){$rootScope.playername=$scope.playername;$rootScope.playerlife=$scope.playerlife;$rootScope.playermoney=$scope.playermoney;$rootScope.playeremail=$scope.playeremail;$rootScope.playingfieldsize=$scope.playingfieldsize}$scope.clickOnLoggin=function(){copyLogginDataToGlobal();checkLogin();$location.path("/game")}});
